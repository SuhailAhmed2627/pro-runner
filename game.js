...
const gameOverMusic = new Audio("Assets/Audio/gameOverMusic.mp3");
...
function touchedObstacle(obstacle) {
   if (
      (obstacle.shape === "circle" && Math.hypot(player.centerCord.x - obstacle.cord.x, player.centerCord.y - obstacle.cord.y) <= player.height - player.radius + obstacle.dimension) ||
      (obstacle.shape === "box" &&
...
function setPowerUp(ability) {
   player.power.poweredUp = !player.power.poweredUp;
   if (ability == "invisibility" || player.power.invisibility) {
      player.power.invisibility = !player.power.invisibility;
      contextObject.globalAlpha = player.power.invisibility ? 0.5 : 1;
   } else if (ability == "slowDown" || player.power.slowedDown) {
      player.power.slowedDown = !player.power.slowedDown;
      GAME.speedX = player.power.slowedDown ? 2 : 5;
   }
   if (player.power.poweredUp) {
      setTimeout(function () {
...
function touchedPowerUp(power) {
   return Math.hypot(player.centerCord.x - power.cord.x, player.centerCord.y - power.cord.y) <= player.height - player.radius + power.dimension ? true : false;
}
